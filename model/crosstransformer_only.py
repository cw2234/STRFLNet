...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input1, enc_input2):
        ...
        enc_outputs = self.enc_self_attn(
            enc_input1, enc_input2, enc_input2
...
class Cross(nn.Module):
    def __init__(self, chan_num=62, class_num=3, Feature_num=64):
        ...
        self.encoder = Encoder(
            n_layers=2, n_heads=8, d_model=self.band_num * 2, d_k=8, d_v=8, d_ff=10
        )
        self.encoder1 = Encoder(
            n_layers=2, n_heads=8, d_model=self.band_num, d_k=8, d_v=8, d_ff=10
        )
        ...
        self.A = torch.rand(
            (1, self.chan_num * self.chan_num), dtype=torch.float32, requires_grad=False
        ).cuda()
