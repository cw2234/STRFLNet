...
# warning 本来是有15个trial的，但每个trial按1s分割成多个样本，这样混在一起导致数据泄露
...
ax.plot(
    range(1, len(train_losses) + 1), train_losses, color=colors1[0], label=" Train Loss"
)
...
ax.plot(
    range(1, len(test_losses) + 1), test_losses, color=colors1[0], label=" Test Loss"
)
